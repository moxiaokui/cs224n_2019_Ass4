...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        sequence = pack_padded_sequence(X, source_lengths)
        output, (last_hidden, last_cell) = self.encoder(sequence)
        enc_hiddens, source_lengths = pad_packed_sequence(output)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1)
        init_decoder_hidden = self.h_projection(last_hidden) #size: b * h
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = F.softmax(e_t, dim=1) #e_t: (b, src_len) dim=1, compute along the row, every coloum,-1 is okay
        a_t = torch.bmm(torch.unsqueeze(alpha_t, 1), enc_hiddens) #shape: (b, 1, 2h)
        a_t = torch.squeeze(a_t, 1)
        U_t = torch.cat((a_t, dec_hidden), dim = 1)
        V_t = self.combined_output_projection(U_t)
        input = torch.tanh(V_t)
        O_t = self.dropout(input)
